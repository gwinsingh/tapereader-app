...
- **Daily manual columns**: Sleep Score (0-100), Readiness Score (0-100), Emotional State (dropdown — legacy, kept for history), Market Bias (dropdown) — user fills these once on the first trade of each day.
- **Psych check-in columns** (daily, auto-filled from the Morning Plan): Energy (1-5, drained ↔ fully charged), Tension (1-5, settled ↔ wired/racing), Urge to Trade Fast? (Yes/No). These replaced Emotional State as the primary psych input — logged pre-market in the Morning Plan form and auto-filled onto EVERY trade of that date (on- or off-plan) via `DAY_FILL_COLS`, keyed by date alone.
