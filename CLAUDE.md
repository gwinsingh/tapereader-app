...
- **74 columns**: Auto-filled trade data, formula columns (Stop after Avg Exit), manual per-trade + daily, enrichment + formula analysis columns (Max R Before Stop, Farthest Price, MAE (R), 1R-6R), market data enrichment, psych check-in columns, the trade-date daily candle (O/H/L/C/V) + volatility references (ATR, 30mATR), and `Sleep (hrs)` (day-level sleep duration). (The live test sheet also has two hand-added manual columns the code does not manage — `RightTheory?` and `EOD Screenshot`; the code reads/writes strictly by header name, so unmanaged columns are ignored.)
...
- **Market data enrichment columns** (auto-filled from Polygon): #1m, #5m, #1H, %Gap, %ATR, RVOL, %VWAP, OR Size ($), OR %ATR, OR High, OR Low, Breakout Vol Ratio, Prior Close Loc, Dist 20 SMA (%), Dist 50 SMA (%), Float, Avg $ Vol, SPY Dir, VIX, PDC, PDH, PDL, O, H, L, C, V, ATR, 30mATR.
- **Trade-date daily candle + volatility references** (auto-filled from Polygon; header strings on the live sheet: `O H L C V ATR 30mATR`): raw daily OHLCV for the trade date plus two pre-open ATR snapshots. `ATR` = daily ATR-14 (mean true range of the 14 sessions **before** the trade date). `30mATR` = mean of the 9:30–10:00 ET range over the 14 sessions **before** the trade date (built from 1-min bars). Both **exclude the trade day** — no lookahead. Storing raw OHLCV keeps the data generic, so future prediction-threshold changes need no re-backfill. The intraday fetch window was widened from 7 to **28 calendar days** before the earliest trade so the 30mATR has ≥14 prior sessions of opening-range data. Young/sparse listings without 14 prior sessions get `N/A` (via `naIfYoung`).
- **Prediction & Execution Skill funnel** (computed server-side in `computeStats`, shown in Performance Overview under the shared filter): a three-stage skill decomposition. **Intra-Day Prediction %** = share of trades whose favorable excursion beyond the open (long: H−O, short: O−L) reached ≥ 1× `30mATR`. **Daily Prediction %** = ≥ 0.8× `ATR` (headline), with ≥ 1.0× `ATR` as the "strong read" secondary. **Execution Skill %** = the Capture Tracker's Target Capture % (among trades whose Max R Before Stop reached the target, mean of min(realized R, target) ÷ target). The capture target is passed from the client (`localStorage` `pct-capture-target`, default 2.5) via the stats route's `?target=` param, so all three recompute for any filtered subset. Denominators (readable-trade counts) are surfaced per metric.
