...
- **Daily Plan tab** (`Date | Symbol | Conviction (1-3) | Thesis | Catalyst`): pre-market watchlist filled via the in-app Morning Plan form (`/pct-bootcamp/trade-journal/plan`). Solves logging conviction/catalyst at the open — captured the night before / pre-market instead. Catalyst uses a dropdown of `CATALYST_OPTIONS`. QQQ/SPY seeded; upsert dedups by symbol.
- **Origin column** (manual dropdown `Watchlist / Callout / Intraday discovery`): auto-filled at CSV upload by matching `date|symbol` against the Daily Plan — **on the plan → `Watchlist`, off-plan → `Intraday discovery`** (`Callout` stays a manual override). Conviction and Catalyst also auto-fill from the plan when blank. **Origin is a separate axis from "Process Followed?"** — idea source must not contaminate the execution-discipline signal.
