...
- **75 columns**: Auto-filled trade data, formula columns (Stop after Avg Exit), manual per-trade + daily, enrichment + formula analysis columns (Max R Before Stop, Farthest Price, MAE (R), 1R-6R), market data enrichment, psych check-in columns, the trade-date daily candle (O/H/L/C/V) + volatility references (ATR, 30mATR, ADR), and `Sleep (hrs)` (day-level sleep duration). (The live test sheet also has two hand-added manual columns the code does not manage — `RightTheory?` and `EOD Screenshot`; the code reads/writes strictly by header name, so unmanaged columns are ignored.)
