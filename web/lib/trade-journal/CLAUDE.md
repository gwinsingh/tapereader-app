...
- **`Capture %` / `In-Window Capture %` floor at 0 and require a real opportunity.** A losing trade captured NONE of the available gain, so it reads 0%, not a negative: "capture" names a fraction of something available and cannot be less than none, and averaging negatives destroys the metric. Separately the denominator must be at least `CAPTURE_MIN_R` (0.25) × initial risk — a peak of $0.16 against $13.78 of risk is not a gain anyone could have captured, and dividing by it manufactures huge percentages out of rounding-scale numbers (2026-09-14 AMD read **−8788%**). Below that threshold the cell is **blank, not zero**, so it drops out of averages instead of dragging them down; `In-Window MFE (R)` already says the trade went nowhere. Before this, 54 of 70 rows were negative and 20 were past ±300%.
