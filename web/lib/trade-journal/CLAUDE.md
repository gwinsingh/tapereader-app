...
A `Daily Plan` tab (`Date | Symbol | Conviction (1-3) | Thesis | Catalyst | L2 Bias`) lets the trader pre-qualify names **before the open**, when calm — solving the "can't log conviction/catalyst/bias at the open" problem. Catalyst is a dropdown of `CATALYST_OPTIONS`; **L2 Bias** (per-symbol order-book read) reuses the `MARKET_BIAS_OPTIONS` (`Bullish`/`Bearish`/`Neutral`). The in-app form is at `/pct-bootcamp/trade-journal/plan` (route: `/api/trade-journal/plan`, GET by date + POST upsert). QQQ/SPY are seeded; the route dedups by symbol so re-typing them is safe.

At CSV upload, `appendTrades` loads the plan map once and, per new trade row, **auto-fills** `Conviction`, `Catalyst`, and `L2 Bias` (each only if blank) and sets the new **`Origin`** column by matching `date|symbol`: a name on the plan → `Watchlist`, anything not on the plan → `Intraday discovery`. `L2 Bias` is a separate trade-sheet column from the daily `Market Bias` (instrument order-book read vs. overall-market read). `Origin` is a deliberately **separate axis from "Process Followed?"** — idea source must not pollute the execution-discipline signal. `Origin` is a manual column (dropdown `Watchlist / Callout / Intraday discovery`, flipped header) the trader can override — e.g. set `Callout` by hand for a vetted callout.
