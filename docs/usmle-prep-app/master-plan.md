...
- [x] P3 (follow-on): **Study Notes** — in-app First-Aid-aligned theory/revision summaries. *(`study_notes` table, migration `0002_study_notes.sql`; CRUD `/api/usmle/notes` + `/api/usmle/notes/[id]`; AI draft gen `/api/usmle/ai/generate-note` via `lib/usmle/notes.ts` (markdown {title,body}, json_schema, adaptive thinking, FA chapter refs not page numbers, no verbatim copying); `components/usmle/StudyNotes.tsx` + dependency-free XSS-safe `Markdown.tsx`; "Notes" nav link. Commit 0568915, deployed + verified live.)*
- [x] Content seed: **24 decks / 288 AI flashcards** for the student's high-yield list (Cardiovascular, Endocrine, Renal, Respiratory), Claude-accuracy-checked, loaded to D1 via `scripts/gen-cards.mjs` + `scripts/cards-to-sql.mjs`. Deck = student subtopic, `topic_id` = taxonomy leaf.
- [x] Study Notes enhancement: **24 pre-built per-subtopic revision notes** (one per high-yield deck) with **Mermaid flowcharts** loaded to D1. Mermaid rendering via lazy client-only `components/usmle/MermaidDiagram.tsx` (out of edge worker; `securityLevel: strict`; code-fallback on parse error) wired into `Markdown.tsx`; `mermaid@11` dep. Scripts: `scripts/gen-notes.mjs` + `scripts/notes-to-sql.mjs`. Commit 1fc651d. (Owner wanted notes ready, not generated on the go.)
- [x] Flashcards iteration 1 (2026-06-23, commit 52a5aad): **bookmarks** (`bookmarked` col, migration 0003; star in review + browser), **Card Browser** (new Browse tab; `GET /api/usmle/cards` filters deck/tag/state/bookmarked/suspended/text; per-card edit/bookmark/suspend/delete; `DELETE /api/usmle/cards/[id]` cascades srs+log), **custom study** (`GET /api/usmle/review/custom`; "Study these N (cram)"; `ReviewSession` now takes a queue|custom source). Cram grades still update FSRS.
- [x] Daily new-card pacing (2026-06-23, commit 36a4162): `app_settings` table (migration 0004) + `lib/usmle/settings.ts` (defaults 20 new/200 reviews per day); `GET/PATCH /api/usmle/settings`; paced `review/queue` (3 pools: urgent uncapped, review-state + new gated by daily caps, "done today" from review_log + client dayStart); `ReviewSession` pacing header + ⚙ limits editor; `TodayPanel` shows paced "cards to review today".
- [ ] Flashcards next wave: **images on cards** (storage decision: Cloudflare R2 vs Drive) → then review stats+streak, leech detection, undo-rating, bulk browser actions, AI "explain card", card↔note links.
...
| 2026-06-22 | ~~Mutations require the write key~~ → **superseded 2026-06-22: USMLE writes decoupled from the project-wide `WRITE_KEY`** (`isAuthorized()` returns `true`; the key stays in use by the 4-Week Challenge). | Single-user app — per-request key management skipped for now. Cost is bounded at the provider level (spend cap); a dedicated `USMLE_WRITE_KEY` can be reintroduced later (see code comment in `lib/usmle/ids.ts`). |
...
| 2026-06-22 | **Built Study Notes** (per-topic markdown summaries, AI-generated First-Aid-aligned). Custom dependency-free markdown renderer (no `react-markdown`) emitting React elements. | Student asked for "4–5 page theory/flowchart" revision content; flashcards alone didn't cover it. Custom renderer keeps zero new deps + edge-safe + XSS-safe vs. pulling a markdown lib. |
| 2026-06-22 | **Pre-seeded 288 high-yield cards** generated by Claude, **accuracy-checked by Claude (not the owner)**, loaded directly to prod D1 (bypassing the in-app approval queue). | Owner is not a clinician and explicitly delegated the medical check to Claude; goal was a ready-to-review deck on day one. Cards tagged `source='ai'`; suspend/delete available. Only correction across 372 generated items: 1 typo + 1 ARNI terminology slip. |
